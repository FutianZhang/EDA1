...
resource "harvester_cloudinit_secret" "cloud-config" {
  ...
  user_data = templatefile("cloud-init.tmpl.yaml", {
      public_key_openssh = [
        data.harvester_ssh_key.mysshkey.public_key,
        file(var.lecturer_public_key_path)
      ]
    })
}
...
resource "harvester_virtualmachine" "mgmtvm" {
  ...
  tags = {
    ...
    condenser_ingress_prometheus_hostname    = "prometheus-${var.username}-1"
    condenser_ingress_prometheus_port        = 9090
    condenser_ingress_nodeexporter_hostname  = "nodeexporter-${var.username}-1"
